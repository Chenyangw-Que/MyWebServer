...
public:
  ...
  // 注册新描述符(如果传入的超时时间大于0 就给此fd绑定一个定时器
  // 以及绑定http连接)
...
private:
  ...
  //std::shared_ptr<httpConnection> http_connections_[kMaxFdNum]; // http连接对象
  std::shared_ptr<BaseConnection> http_connections_[kMaxFdNum];
  TimerHeap timer_heap_; //定时器小顶堆
