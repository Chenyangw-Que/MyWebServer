...
public:
  ...
  // std::shared_ptr<httpConnection> holder() {
  //   std::shared_ptr<httpConnection> http(holder_.lock());
  //   return http;
  // }

  // void set_holder(std::shared_ptr<httpConnection> holder) { holder_ = holder; }

    std::shared_ptr<BaseConnection> holder() {
    std::shared_ptr<BaseConnection> http(holder_.lock());
    return http;
  }

  void set_holder(std::shared_ptr<BaseConnection> holder) { holder_ = holder; }
...
private:
  ...
  int last_events_; //上一此事件（主要用于记录如果本次事件和上次事件一样
                    //就没必要调用epoll_mod）

  // weak_ptr是一个观测者（不会增加或减少引用计数）,同时也没有重载->,和*等运算符
  // 所以不能直接使用
  //可以通过lock函数得到它的shared_ptr（对象没销毁就返回，销毁了就返回空shared_ptr）
  // expired函数判断当前对象是否销毁了
  
  //std::weak_ptr<httpConnection> holder_;
  std::weak_ptr<BaseConnection> holder_;
