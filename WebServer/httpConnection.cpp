...
using namespace httpDefine;
//  httpConnection类 给连接套接字绑定事件处理回调
httpConnection::httpConnection(EventLoop *event_loop, int connect_fd)
    : event_loop_(event_loop), connect_fd_(connect_fd),
      connect_channel_(new Channel(connect_fd)), connection_state_(CONNECTED),
      process_state_(STATE_PARSE_LINE), parse_state_(START),
      method_(METHOD_GET), version_(HTTP_1_1), is_error_(false),
      is_keep_alive_(false)
{
  // 给连接套接字绑定读、写、连接的回调函数
  connect_channel_->set_read_handler(
      std::bind(&httpConnection::HandleRead, this));
  connect_channel_->set_write_handler(
      std::bind(&httpConnection::HandleWrite, this));
  connect_channel_->set_update_handler(
      std::bind(&httpConnection::HandleUpdate, this));
...
// http连接析构时 关闭连接套接字
httpConnection::~httpConnection() { close(connect_fd_); }
...
// 从epoll事件表中删除fd(绑定的定时器被删除时
// 会调用此函数),然后http连接释放，会close掉fd 注意此时需要延长  httpConnection
// 的寿命，避免  httpConnection 在函数执行过程中主动关闭 导致 coredump
...
// 处理读   读请求报文数据到 read_buffer 解析请求报文
...
void httpConnection::HandleRead()
{
  ...
  do
  {
    ...
    LOG(INFO) << "Request " << read_buffer_;
    // 第一次读肯定不DISCONNECTING
    ...
    if (process_state_ == STATE_PARSE_LINE)
    {
      ...
      else if (line_state == PARSE_LINE_ERROR)
      {
        ...
        LOG(WARNING) << "Parse request line error, sockfd: " << connect_fd_
                     << ", " << read_buffer_ << "******";
        read_buffer_.clear();
        // 标记为error的都是直接返回错误页面
        is_error_ = true;
    ...
    if (process_state_ == STATE_PARSE_HEADERS)
    {
      ...
      {
        LOG(WARNING) << "Parse request header error, sockfd: " << connect_fd_
                     << ", " << read_buffer_ << "******";
        //标记为error的都是直接返回错误页面
        is_error_ = true;
        ReturnErrorMessage(connect_fd_, 400, "Bad Request");
    ...
    if (process_state_ == STATE_RECV_BODY)
    {
      ...
      else
      {
        // 标记为error的都是直接返回错误页面
        is_error_ = true;
        ReturnErrorMessage(connect_fd_, 400,
                           "Bad Request: Lack of argument (Content-length)");
        break;
      }
      // 
      if (read_buffer_.size() < content_length-1)
      {
        break;
      }
      process_state_ = STATE_RESPONSE;
...
void httpConnection::HandleUpdate()
{
  ...
  else
  {
    ...
    event_loop_->RunInLoop(
        std::bind(&httpConnection::Delete, shared_from_this()));
  }
}

// 处理错误（返回错误信息）
void httpConnection::ReturnErrorMessage(int fd, int error_code,
                                        std::string error_message)
{
  ...
  response_body +=
      "<div align=\"center\">" + std::to_string(error_code) + error_message;
  response_body += "<hr>lawson's web server\n</div></body></html>";

  //响应头
  std::string response_header;
  response_header +=
      "HTTP/1.1 " + std::to_string(error_code) + error_message + "\r\n";
...
LineState httpConnection::ParseRequestLine()
{
  ...
  // GET /filename HTTP1.1 请求文件名， 找到请求资源
...
HeaderState httpConnection::ParseRequestHeader()
{
  ...
  for (; i < request_data.size() && !is_finish; ++i)
  {
    ...
    case CR:
    {
      ...
      {
        parse_state_ = LF;
        std::string key(request_data.begin() + key_start,
                        request_data.begin() + key_end);
        std::string value(request_data.begin() + value_start,
                          request_data.begin() + value_end);
...
ResponseState httpConnection::BuildResponse()
{
  ...
  if (method_ == METHOD_POST)
  {
         ...
         request_headers_["Connection"] == "keep-alive"))
    {
      ...
      responseHeader +=
          ...
          "Keep-Alive: timeout=" + std::to_string(kDefaultKeepAliveTime) +
          "\r\n";
    }
    // 通过fileName解析cgi
    // 为了方便我这里默认post只访问cgi
    int pos = file_name_.find('.');
    string cgi = file_name_.substr(0,pos);
    // 现在约定前两位表示请求小类，后几位表示请求大类
    int categ = int(10 * (cgi[0]-'0')+ cgi[1]-'0');
    cgi = cgi.substr(2);
    
    if(cgi == "upload"){
      if(categ ==1){
        write_buffer_ = processImage(read_buffer_);
  ...
  else if (method_ == METHOD_GET || method_ == METHOD_HEAD)
  {
    ...
    //     std::to_string(sizeof(web_server_favicon)) + "\r\n"; response_header
    //     += "Server: lawson's webserver\r\n"; response_header += "\r\n";
    //     write_buffer_ += response_header;
    //     write_buffer_ += std::string(web_server_favicon, web_server_favicon +
    //     sizeof(web_server_favicon)); return RESPONSE_SUCCESS;
    // }
    ...
    // // // 先在缓存中找
    // if (!LFUCache::GetInstance().Get(file_name_, write_buffer_)) {
    //   // cache miss.
    //   // 请求的文件不存在返回404
    ...
    //   write_buffer_ +=
    //       std::string(file_address, file_address + file_stat.st_size);

    //   // 加入缓存
    //   std::string context(file_address, file_stat.st_size);
    //   LFUCache::GetInstance().Set(file_name_, context);
    ...
    void *mmapRet = mmap(NULL, file_stat.st_size, PROT_READ, MAP_PRIVATE,
                         src_fd, 0);
    close(src_fd);
    if (mmapRet == (void *)-1)
    {
      munmap(mmapRet, file_stat.st_size);
      write_buffer_.clear();
      ReturnErrorMessage(connect_fd_, 404, "Not Found!");
      return RESPONSE_ERROR;
    }
    char *src_addr = static_cast<char *>(mmapRet);
    write_buffer_ += std::string(src_addr, src_addr + file_stat.st_size);
    ;
    munmap(mmapRet, file_stat.st_size);
    return RESPONSE_SUCCESS;
  }
