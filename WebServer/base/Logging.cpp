...
//运行异步日志线程(只在第一次调用时运行,将buffer数据写入日志文件),
//写日志到buffer中
...
Logging::Logging(const char *file_name, int line, int level)
  ...
  impl_.stream_ << impl_.time_str_ << " " << impl_.file_name_ << ':'
...
Logging::~Logging() {
  if (impl_.level_ >= min_log_level_) {
    ...
    //写日志
    if (open_log) {
      WriteLog(buffer.buffer(), buffer.buffSize(), impl_.is_fatal_);
    }
    //日志输出到标准错误流
    if (log_to_stderr_) {
      if (color_log_to_stderr_) {
        std::cout << impl_.log_color_
                  << std::string(buffer.getBuffer(), buffer.buffSize()) << COLOR_END;
      } else {
        std::cout << std::string(buffer.getBuffer(), buffer.buffSize());
      }
    }
...
Logging::Impl::Impl(const char *file_name, int line, int level)
    : stream_(), file_name_(file_name), line_(line), level_(level),
      is_fatal_(false) {
  //文件名只取最后面的
...
void Logging::Impl::FormatTime() {
  ...
  // time函数一样的效果 返回一个time_t类型表示秒
  // 然后使用localtime(&time_t)获得当前时间 类型为tm
  struct timeval now;
