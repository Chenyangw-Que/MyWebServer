...
void KeyList::init(int freq) {
  freq_ = freq;
  // Dummyhead_ = tail_= new ListNode<Key>;
  Dummyhead_ = QueMemory::newElement<ListNode<Key>>();
  tail_ = Dummyhead_;
...
bool KeyList::isEmpty() { return Dummyhead_ == tail_; }

key_node KeyList::getLast() { return tail_; }
...
void LFUCache::Init() {
  ...
  // Dummyhead_ = new ListNode<KeyList>();
  Dummyhead_ = QueMemory::newElement<ListNode<KeyList>>();
...
void LFUCache::AddFreq(key_node &nowk, freq_node &nowf) {
  ...
  if (nowf->getNext() == nullptr ||
    ...
    // nxt = new ListNode<KeyList>();
    nxt = QueMemory::newElement<ListNode<KeyList>>();
...
void LFUCache::Set(string &key, string &val) {
  ...
  if (kmap_.size() == capacity_) {
    ...
    // printf("need to delete a ListNode\n");
    ...
    // delete last;
    QueMemory::deleteElement(last);
    // 如果频度最小的链表已经没有节点，就删除
    if (head->getValue().isEmpty()) {
      Del(head);
    }
  }
  // key_node nowk = new ListNode<Key>();
  key_node nowk = QueMemory::newElement<ListNode<Key>>();
