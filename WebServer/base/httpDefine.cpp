...
pthread_once_t httpDefine::MimeType::once_control = PTHREAD_ONCE_INIT;
std::unordered_map<std::string, std::string> httpDefine::MimeType::mime_map;

//HTML协议的文件类型
void httpDefine::MimeType::OnceInit() {
...
//根据type得到mime文件类型
std::string httpDefine::MimeType::get_mime(const std::string& type) {
    //只会执行一次的函数
    pthread_once(&once_control, httpDefine::MimeType::OnceInit);
